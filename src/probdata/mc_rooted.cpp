...
void buildRootMcModel(Scip **s, const Graph *g, NetworKit::node sun, bool allBinary) {
    assert(s != nullptr);

    SCIP_CALL_EXC(SCIPcreate(s))
    auto scip = *s;
    // add prob data
    auto *probdata = new ProbDataRootedMc(scip, g, sun);
    SCIP_CALL_EXC(SCIPcreateObjProb(scip, "mc sun model",
                                    probdata, true))
    ...
    for (auto u: g->nodeRange()) {
        if (u != sun) {
            ...
            varname << "x_" << sun << "-" << u;
            auto weight = g->hasEdge(sun, u) ? g->weight(sun, u) : 0;
            ...
            probdata->addVariable(sun, u, var);
        }
    }
