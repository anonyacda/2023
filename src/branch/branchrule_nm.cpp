...
int64_t BranchruleNM::degreeBinary(SCIP_VAR *variable) {
    ...
    auto u = probDataSunMc->varToNode(variable);

    int64_t degree = 0;
    for (auto neighbor: probDataSunMc->getMcGraph()->neighborRange(u)) {
        if (neighbor != sun) {
            SCIP_VAR *neighborVar = probDataSunMc->nodeToVar(neighbor);
...
SCIP_RETCODE BranchruleNM::scip_execlp(SCIP *scip, SCIP_BRANCHRULE */*branchrule*/, unsigned int /*allowaddcons*/,
    ...
    *result = SCIP_DIDNOTRUN;
    ...
    for (auto u: probDataSunMc->getMcGraph()->nodeRange()) {
        if (u != probDataSunMc->getSun()) {
            degreeSum += probDataSunMc->getMcGraph()->degree(u);
        }
    }
    double avgDegree =
            static_cast<double>(degreeSum) / static_cast<double>(probDataSunMc->getMcGraph()->numberOfNodes());
